...
$result = az deployment sub create --name "Deploy-$(Get-Random)" --template-file ./infra/main.bicep --location $Location --parameters environmentName="$EnvironmentName" location="$Location" | ConvertFrom-Json
